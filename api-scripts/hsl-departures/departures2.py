...
def get_hsl_departures(stop_ids):
    ...
    if not API_KEY or not API_ENDPOINT:
        # response = "Error: Required environment variables are missing.\nPlease ensure API_KEY and API_ENDPOINT are set in your .env file"
        responseValue = concatResponse(responseValue, "Error: Required environment variables are missing.")
        responseValue = concatResponse(responseValue, "Please ensure API_KEY and API_ENDPOINT are set in your .env file")
    ...
    try:
        ...
        if "data" not in data or data["data"]["stops"] is None:
            responseValue = concatResponse(responseValue, "Error: Could not retrieve stop data. Please check the stop ID or API key.")
            return responseValue
        ...
        for s in stop_data:
            ...
            responseValue = concatResponse(responseValue, f"\n{vehicle_mode} {stop_name}:") # Shortened text length

            if not departures:
                responseValue = concatResponse(responseValue, "No upcoming departures found.")
                return responseValue
            ...
            for i, departure in enumerate(departures):
                ...
                responseValue = concatResponse(responseValue, f"{transit_number} at {departure_time} - ({time_to_departure})") # Super-short text length
        ...
        if e.response and e.response.status_code == 401:
            responseValue = concatResponse(responseValue, "Error: 401 Unauthorized. The API key is likely missing, incorrect, or expired.")
        elif e.response and e.response.status_code == 404:
            responseValue = concatResponse(responseValue, "Error: 404 Not Found. The API endpoint may be incorrect or deprecated.")
            responseValue = concatResponse(responseValue, "The code has been updated to use the correct endpoint. Please try again.")
        else:
            responseValue = concatResponse(responseValue, f"An error occurred while connecting to the API: {e}")
    except json.JSONDecodeError:
        responseValue = concatResponse(responseValue, "Error: The API response was not valid JSON.")
    except KeyError as e:
        responseValue = concatResponse(responseValue, f"Error: Missing key in API response data: {e}")
...
def concatResponse(responseValue, newValue):
    if(responseValue != ""):
        responseValue = responseValue + "\n"
    responseValue = responseValue + newValue
    return responseValue
